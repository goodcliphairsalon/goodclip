// ─────────────────────────────────────────
//  CONFIG – thay SCRIPT_URL sau khi deploy
// ─────────────────────────────────────────
const CFG = {
  SCRIPT_URL: "PASTE_YOUR_APPS_SCRIPT_URL_HERE",
  stylist:    "Hanh Dao",
  openDays:   [2, 3, 4, 5, 6],   // Tue–Sat  (0=Sun … 6=Sat)
  openHour:   10,
  closeHour:  19,                 // 7 PM
  slotMin:    15,                 // 15-min blocks
};
...
const SERVICES = {
  "✂️ Hair Services": [
    ...
    { id:"womens-cut",     name:"Women's Haircut",                    price:"$32–$35",  base:32,  dur:30  },
    { id:"womens-shampoo", name:"Women's Shampoo Add-on",             price:"$7–$10",   base:7,   dur:15  },
    ...
    items.forEach(svc => {
      ...
      el.innerHTML = `
        ...
        <div class="svc-info">
          ...
          <div class="svc-dur">${svc.dur} min</div>
        </div>
...
function refreshSummary() {
  ...
  const mins  = svcs.reduce((t, s) => t + s.dur, 0);
  const total = svcs.reduce((t, s) => t + s.base, 0);
  bar.classList.add("visible");
  bar.innerHTML = `
    <span>${svcs.length} service${svcs.length > 1 ? "s" : ""} &nbsp;·&nbsp; ${mins} min</span>
